...
namespace JXTPortal.EmailSender
{
    ...
    public class SmtpSender : IRichMessageEmailSender
    {
        ...
        public SmtpSender(SmtpDeliveryMethod method, string directory)
        {
            _smtpClient = new SmtpClient();
            _smtpClient.DeliveryMethod = method;
            _smtpClient.PickupDirectoryLocation = directory;
        }
        ...
        public void Send(Message message, string bouncebackemail = "")
        {
            ...
            if (_asyncSend)
            {
                ...
                MailMessage msg = CreateMailMessage(message, bouncebackemail);
            ...
            {
                using (MailMessage msg = CreateMailMessage(message, bouncebackemail))
                {
        ...
        private static MailMessage CreateMailMessage(Message message, string bouncebackemail = "")
        {
            ...
            {
                mailMessage.Subject = mailMessage.Subject.Replace("{" + (string)entry.Key + "}", entry.Value ==  null ? string.Empty : entry.Value.ToString() );
                mailMessage.Body = mailMessage.Body.Replace("{" + (string)entry.Key + "}", entry.Value == null ? string.Empty : entry.Value.ToString());
            }
            ...
            if (!string.IsNullOrWhiteSpace(bouncebackemail))
            {
                mailMessage.ReplyToList.Add(new MailAddress(bouncebackemail));
                mailMessage.DeliveryNotificationOptions = DeliveryNotificationOptions.OnFailure;
                //mailMessage.Headers.Add("Disposition-Notification-To", bouncebackemail);
            }

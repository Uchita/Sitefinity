...
module.exports = function (grunt) {
    ...
    // Custom variable //Amresh
    const currentPath = path.resolve(__dirname);
    const mainDirectoryPath = path.join(currentPath, "../../../");
    const projectDirectoryName = currentPath.split("\\").slice(-1)[0];
    const projectDirSitefinity = path.join(mainDirectoryPath, 'SitefinityWebApp/ResourcePackages/' + projectDirectoryName);
    ...
    // Fonts file not availble that's why i have comment the below code
    ...
    grunt.initConfig({
        ...
        //SitefinityWebApp ResourcePackage
        SWebAppResourcePkg: {
            dist: {
                path: projectDirSitefinity + '/assets/dist'
            },
            path: projectDirSitefinity
        },
        ...
        sass: {
            ...
            dist: {
                ...
                },
                {
                    ...
                    cwd: '<%= src.path %>/' + projectAssetsFolder + '/sass/',
                    src: ['*.scss'],
                    dest: '<%= SWebAppResourcePkg.dist.path %>/css/',
                    ext: '.css'
                }
                ]
            }
        ...
        copy: {
            dist: {
                files: [
                    ...
                    { expand: true, cwd: 'assets/src/' + projectAssetsFolder + '/js', src: '**', dest: 'assets/dist/js/' }
                ]
            }
        },
        ...
        uglify: {
            ...
            dist: {
                files: [{
                    ...
                    '<%= SWebAppResourcePkg.dist.path %>/js/project.min.js': projectJsfiles
                }]
            }
        },
        ...
        browserSync: {
            dev: {
                bsFiles: {
                    src: [
                        '<%= SWebAppResourcePkg.dist.path %>/css/*.css',
                        '<%= SWebAppResourcePkg.dist.path %>/js/*.js',
                        '<%= SWebAppResourcePkg.path %>/MVC/**/*.cshtml'
                    ]
                },
                options: {

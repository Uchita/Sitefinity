...
namespace JXTNext.Sitefinity.Widgets.Job.Mvc.Controllers
{
    ...
    public class JobDetailsController : Controller
    {
        ...
        public ActionResult Index(int? jobId)
        {
            ...
            if (jobId.HasValue)
            {
                ...
                if (jobListingResponse.Job != null && !jobListingResponse.Job.IsDeleted)
                {
                    ...
                    viewModel.ClassificationFilters = JobDetailsViewModel.CreateClassificationFilters(classifOrdDict);
                    viewModel.LocationFilters = JobDetailsViewModel.CreateLocationFilters(locOrdDict);
                    ...
                    ViewBag.GoogleForJobs = GenerateJobPostingSchema(jobListingResponse);
        ...
        private string GenerateJobPostingSchema(IGetJobListingResponse jobListingResponse)
        {
            var identifier = new Identifier();
            identifier.schemaType = "PropertyValue";
            identifier.name = jobListingResponse.Job.CustomData.ContainsKey("ContactDetails") ? jobListingResponse.Job.CustomData["ContactDetails"] : null;
            identifier.value = jobListingResponse.Job.ReferenceNo;

            var hiringOrganization = new HiringOrganization();
            hiringOrganization.schemaType = "Organization";
            hiringOrganization.name = jobListingResponse.Job.CustomData.ContainsKey("CompanyName") ? jobListingResponse.Job.CustomData["CompanyName"] : null;
            hiringOrganization.sameAs = "";
            hiringOrganization.logo = "";

            var address = new Address();
            address.schemaType = "PostalAddress";
            address.streetAddress = jobListingResponse.Job.CustomData.ContainsKey("CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value") ? jobListingResponse.Job.CustomData["CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value"] : null;
            address.addressLocality = jobListingResponse.Job.CustomData.ContainsKey("CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value") ? jobListingResponse.Job.CustomData["CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value"] : null;
            address.addressRegion = jobListingResponse.Job.CustomData.ContainsKey("CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value") ? jobListingResponse.Job.CustomData["CountryLocationArea[0].Filters[0].SubLevel[0].SubLevel[0].Value"] : null;
            address.postalCode = "";
            address.addressCountry = jobListingResponse.Job.CustomData.ContainsKey("CountryLocationArea[0].Filters[0].SubLevel[0].Value") ? jobListingResponse.Job.CustomData["CountryLocationArea[0].Filters[0].SubLevel[0].Value"] : null;

            var jobLocation = new JobLocation();
            jobLocation.schemaType = "Place";
            jobLocation.address = address;
            
            var value = new JobPostingValue();
            value.schemaType = "QuantitativeValue";

            if (jobListingResponse.Job.CustomData.ContainsKey("Salary.Filters[0].Min") && !string.IsNullOrEmpty(jobListingResponse.Job.CustomData["Salary.Filters[0].Min"]))
                value.minValue = Convert.ToDecimal(jobListingResponse.Job.CustomData["Salary.Filters[0].Min"]);
            else
                value.minValue = null;

            if (jobListingResponse.Job.CustomData.ContainsKey("Salary.Filters[0].Max") && !string.IsNullOrEmpty(jobListingResponse.Job.CustomData["Salary.Filters[0].Max"]))
                value.maxValue = Convert.ToDecimal(jobListingResponse.Job.CustomData["Salary.Filters[0].Max"]);
            else
                value.maxValue = null;

            var quantitiveValue = jobListingResponse.Job.CustomData.ContainsKey("Salary.Filters[0].Value") ? jobListingResponse.Job.CustomData["Salary.Filters[0].Value"] : null;

            if (!string.IsNullOrEmpty(quantitiveValue))
            {
                if (quantitiveValue.ToLower().Contains("annual"))
                    quantitiveValue = "YEAR";
                else if (quantitiveValue.ToLower().Contains("day"))
                    quantitiveValue = "DAY";
                else if (quantitiveValue.ToLower().Contains("daily"))
                    quantitiveValue = "DAY";
                else if (quantitiveValue.ToLower().Contains("hour"))
                    quantitiveValue = "HOUR";
                else if (quantitiveValue.ToLower().Contains("week"))
                    quantitiveValue = "WEEK";
                else if (quantitiveValue.ToLower().Contains("year"))
                    quantitiveValue = "YEAR";

            }
            var employmentType = jobListingResponse.Job.CustomData.ContainsKey("WorkType.Filters[0].Value") ? jobListingResponse.Job.CustomData["WorkType.Filters[0].Value"] : null;

            if (!string.IsNullOrEmpty(employmentType))
            {
                if (employmentType.ToLower().Contains("full"))
                    employmentType = "FULL_TIME";
                else if (employmentType.ToLower().Contains("permanent"))
                    employmentType = "FULL_TIME";
                else if (employmentType.ToLower().Contains("part"))
                    employmentType = "PART_TIME";
                else if (employmentType.ToLower().Contains("contract"))
                    employmentType = "CONTRACTOR";
                else if (employmentType.ToLower().Contains("temp"))
                    employmentType = "TEMPORARY";
                else if (employmentType.ToLower().Contains("vol"))
                    employmentType = "VOLUNTEER";
                else if (employmentType.ToLower().Contains("diem"))
                    employmentType = "PER_DIEM";
                else
                    employmentType = "OTHER";
            }
            value.unitText = quantitiveValue;

            var baseSalary = new BaseSalary();
            baseSalary.schemaType = "MonetaryAmount";
            baseSalary.currency = "$";
            baseSalary.value = value;

            var jobPostingSchemaRoot = new JobPostingSchemaRoot();
            jobPostingSchemaRoot.context = @"https://schema.org/";
            jobPostingSchemaRoot.schemaType = "JobPosting";
            jobPostingSchemaRoot.title = jobListingResponse.Job.Title;
            jobPostingSchemaRoot.description = jobListingResponse.Job.Description;
            jobPostingSchemaRoot.identifier = identifier;
            ...
            jobPostingSchemaRoot.datePosted = string.Format("{0}+{1}", utcTime.ToString("yyyy-MM-ddThh:mm:ss"), offset.Hours.ToString("00") + ":" + offset.Minutes.ToString("00"));
            jobPostingSchemaRoot.validThrough = (jobListingResponse.Job.ExpiryDate.HasValue) ? string.Format("{0}+{1}", eutcTime.ToString("yyyy-MM-ddThh:mm:ss"), eoffset.Hours.ToString("00") + ":" + eoffset.Minutes.ToString("00")) : string.Empty;
            jobPostingSchemaRoot.employmentType = employmentType;
            jobPostingSchemaRoot.hiringOrganization = hiringOrganization;
            jobPostingSchemaRoot.jobLocation = jobLocation;
            jobPostingSchemaRoot.baseSalary = baseSalary;
            ...
            return @"<script type='application/ld+json'> " + JsonConvert.SerializeObject(jobPostingSchemaRoot, settings).Replace(@"\r", string.Empty).Replace("schemaType", "@type") + " </script>";
        }

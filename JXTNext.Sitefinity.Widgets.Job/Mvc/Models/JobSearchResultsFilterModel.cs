...
namespace JXTNext.Sitefinity.Widgets.Job.Mvc.Models
{
    public class JobSearchResultsFilterModel
    {
        ...
        /// <summary>
        /// Map filter slugs to respective Guids.
        /// This creates the expected filters in the model to make the search work.
        /// </summary>
        /// <param name="connector"></param>
        /// <param name="filterModel"></param>
        public static void MapFilterSlugsToIds(IOptionsConnector connector, JobSearchResultsFilterModel filterModel)
        {
            JXTNext_GetJobFiltersRequest request = new JXTNext_GetJobFiltersRequest();
            IGetJobFiltersResponse filtersResponse = connector.JobFilters<JXTNext_GetJobFiltersRequest, JXTNext_GetJobFiltersResponse>(request);

            string slugs;

            char[] charsToTrim = { JobConstants.FilterItemsDelimeter, ' ' };

            if (filterModel.Filters == null)
            {
                filterModel.Filters = new List<JobSearchFilterReceiver>();
            }

            // process filters for classification/sub-classification
            slugs = filterModel.Classification + JobConstants.FilterItemsDelimeter + filterModel.SubClassification;
            slugs = slugs.Trim(charsToTrim);
            if (slugs != "")
            {
                ProcessSlugs(filtersResponse, filterModel, JobConstants.FilterRootClassifications, slugs);
            }

            // process filters for country/location/area
            slugs = filterModel.Country + JobConstants.FilterItemsDelimeter + filterModel.Location + JobConstants.FilterItemsDelimeter + filterModel.Area;
            slugs = slugs.Trim(charsToTrim);
            if (slugs != "")
            {
                ProcessSlugs(filtersResponse, filterModel, JobConstants.FilterRootCountryLocationArea, slugs);
            }

            // process filters for work types
            if (!string.IsNullOrWhiteSpace(filterModel.WorkType))
            {
                ProcessSlugs(filtersResponse, filterModel, JobConstants.FilterRootWorkType, filterModel.WorkType);
            }

            // process filters for companies
            if (!string.IsNullOrWhiteSpace(filterModel.CompanyName))
            {
                ProcessSlugs(filtersResponse, filterModel, JobConstants.FilterRootCompanyName, filterModel.CompanyName);
            }

            // process filters for salary
            if (!string.IsNullOrWhiteSpace(filterModel.SalaryType))
            {
                ProcessSalarySlugs(filtersResponse, filterModel);
            }
        }

        /// <summary>
        /// Process slugs related to hierarichal and flat taxonomies and create the expected filters.
        /// </summary>
        /// <param name="filtersResponse"></param>
        /// <param name="filterModel"></param>
        /// <param name="rootId"></param>
        /// <param name="slugs"></param>
        private static void ProcessSlugs(IGetJobFiltersResponse filtersResponse, JobSearchResultsFilterModel filterModel, string rootId, string slugs)
        {
            // the root must be present in available filters
            var jobFilterRoot = filtersResponse.Filters.Data.Find(f => f.Name == rootId);
            if (jobFilterRoot == null || jobFilterRoot.Filters == null)
            {
                return;
            }

            // do not create the filter root if it already exist.
            // search fails when there are duplicate roots.
            if (filterModel.Filters.Find(f => f.rootId == rootId) != null)
            {
                return;
            }

            var receiver = new JobSearchFilterReceiver();
            receiver.rootId = rootId;
            receiver.values = new List<JobSearchFilterReceiverItem>();
            filterModel.Filters.Add(receiver);

            foreach (var slug in slugs.Split(JobConstants.FilterItemsDelimeter))
            {
                ProcessSlugsToId(jobFilterRoot.Filters, receiver.values, slug);
            }
        }

        /// <summary>
        /// Process slug and value related to salary and create the expected filters.
        /// </summary>
        /// <param name="filtersResponse"></param>
        /// <param name="filterModel"></param>
        private static void ProcessSalarySlugs(IGetJobFiltersResponse filtersResponse, JobSearchResultsFilterModel filterModel)
        {
            var rootId = JobConstants.FilterRootSalary;
            ...
            var jobFilter = jobFilterRoot.Filters.Find(f => f.Slug == filterModel.SalaryType);
            if (jobFilter != null)
            {
                var salaryReceiver = new JobSearchSalaryFilterReceiver();
                salaryReceiver.RootName = rootId;
                salaryReceiver.TargetValue = jobFilter.ID;
                salaryReceiver.LowerRange = filterModel.SalaryMin;
                salaryReceiver.UpperRange = filterModel.SalaryMax;

                filterModel.Salary = salaryReceiver;
            }
        }

        /// <summary>
        /// Process each slug and it's hierarchy to find the respective Guid and add to the search filter.
        /// </summary>
        /// <param name="jobFilters"></param>
        /// <param name="filters"></param>
        /// <param name="slug"></param>
        /// <param name="slugPrefix"></param>
        private static void ProcessSlugsToId(List<JobFilter> jobFilters, List<JobSearchFilterReceiverItem> filters, string slug, string slugPrefix = "")
        {
            if (jobFilters == null)
            {
                return;
            ...
            var parts = slug.Split(new[] { JobConstants.FilterHierarchyDelimeter }, 2);

            var slugCheck = slugPrefix + parts[0];

            var jFilter = jobFilters.Find(f => f.Slug == slugCheck);
            if (jFilter == null)
            {
                return;
            }

            var receiverItem = filters.Find(i => i.ItemID == jFilter.ID);
            if (receiverItem == null)
            {
                receiverItem = new JobSearchFilterReceiverItem
                {
                    ItemID = jFilter.ID
                };

                filters.Add(receiverItem);
            }

            if (parts.Length > 1)
            {
                if (receiverItem.SubTargets == null)
                {
                    receiverItem.SubTargets = new List<JobSearchFilterReceiverItem>();
                }

                ProcessSlugsToId(jFilter.Filters, receiverItem.SubTargets, parts[1], slugCheck + JobConstants.FilterHierarchyDelimeter);
            }
        }
    ...
    public class JobSearchResultsFilterBinder : IModelBinder
    {
        ...
        public object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            ...
            targetModel.Classification = request.Params[JobConstants.FilterParamClassification];
            targetModel.SubClassification = request.Params[JobConstants.FilterParamSubClassification];
            targetModel.Country = request.Params[JobConstants.FilterParamCountry];
            targetModel.Location = request.Params[JobConstants.FilterParamLocation];
            targetModel.Area = request.Params[JobConstants.FilterParamArea];
            targetModel.WorkType = request.Params[JobConstants.FilterParamWorkType];
            targetModel.SalaryType = request.Params[JobConstants.FilterParamSalaryType];
            targetModel.CompanyName = request.Params[JobConstants.FilterParamCompanyName];

            int salaryValue = 0;

            if (int.TryParse(request.Params[JobConstants.FilterParamSalaryMin], out salaryValue))
            {
                targetModel.SalaryMin = salaryValue;
            }

            if (int.TryParse(request.Params[JobConstants.FilterParamSalaryMax], out salaryValue))
            {
                targetModel.SalaryMax = salaryValue;
            }

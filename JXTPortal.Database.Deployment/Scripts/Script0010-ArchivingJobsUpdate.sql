...
AS    
 ...
 DECLARE @iReturnCode  int,    
   ...
   @iCurrentRowId int,    
            @iLoopControl int    
 DECLARE @PostponeDate INT = 30
  
 SELECT @iLoopControl = 1    
    
 SELECT @iNextRowId = MIN(JobID)    
 FROM   Jobs (NOLOCK)    
 WHERE DATEADD(DD, @PostponeDate, ExpiryDate) < CAST(GETDATE() AS DATE)    
 OR Expired = 1  
 ...
 BEGIN    
   ...
   AND   (DATEADD(DD, @PostponeDate, ExpiryDate) < CAST(GETDATE() AS DATE) AND Expired = 0) -- Only live jobs  
...
*/
 ...
 SELECT @IsJobExpired = 1 FROM Jobs NOLOCK 
	WHERE JobID = @JobID AND  
			((Expired = 0 AND DATEADD(DD, @PostponeDate, ExpiryDate) < CAST(GETDATE() AS DATE)) OR Expired = 1)  -- Live job which is expired or checked expired
 ...
 UPDATE Jobs SET              
  ...
  SiteID = @SiteID, WorkTypeID = WorkType, JobName = Title, [Description] = ShortDescription, FullDescription = #FlatXML.FullDescription,               
   WebServiceProcessed = 1, ApplicationEmailAddress = ISNULL(#FlatXML.ApplicationEmailAddress,''), Visible = 1,                       
      ...
      Expired = 0, ExpiryDate = CASE WHEN (ExpiryDate < GETDATE()) THEN DATEADD(DD, 30, GETDATE()) ELSE ExpiryDate END
 FROM               

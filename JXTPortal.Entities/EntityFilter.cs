...
namespace JXTPortal.Entities
{
        ...
        where T : ListBase<Entity>
	{
		...
		private bool IsOk(string ObjectPropertyValue , string Operator, string UserValue)
		{
			...
			{
				throw new System.Exception("The operator '" + Operator + "' does not match the type String !");
			}
			return Rt;
		}
		...
		private bool IsOk(int ObjectPropertyValue , string Operator, int UserValue)
		{
			...
			{
				throw new System.Exception("The operator '" + Operator + "' does not match the type int !");
			}
			return Rt;
		}
		...
		private bool IsOk(Guid ObjectPropertyValue , string Operator, Guid UserValue)
		{
			...
			{
				throw new System.Exception("The operator '" + Operator + "' does not match the type Guid !");
			}
			return Rt;
		}
		...
		private bool IsOk(double ObjectPropertyValue , string Operator, double UserValue)
		{
			...
			else
			{
				...
				throw new System.Exception("The operator '" + Operator + "' does not match the type double !");
			}
			return Rt;
		}
		...
		private bool IsOk(decimal ObjectPropertyValue , string Operator, decimal UserValue)
		{
			...
			else
			{
				...
				throw new System.Exception("The operator '" + Operator + "' does not match the type decimal !");
			}
			return Rt;			
		}
		...
		private bool IsOk(DateTime ObjectPropertyValue , string Operator, DateTime UserValue)
		{
			...
			else
			{
				...
				throw new System.Exception("The operator '" + Operator + "' does not match the type DateTime !");
			}
			return Rt;
		}
		...
		private bool IsOk(object ObjectPropertyValue , string Operator, string UserValue)
		{
			...
			if (Operator.ToUpper().Equals("IN"))
			{
				...
				catch (System.Exception ex) { throw new InvalidFilterCriteriaException("Invalid IN Filter value", ex); }
			} 
			else if (TypeOfValue ==typeof(string))
			{
			...
			{
				throw new System.Exception("Filtering is not possible on the type " + TypeOfValue.ToString());
			}
			return Rt;
		}
		#endregion 
        ...
        public void ApplyFilter(T ObjectToFilter, string StrFilter) 
		{
			...
			for(int x = 0; x < ListOfExpressions.Count; x++)
			{
				...
				if( PropsInfo[x] == null) 
				{
					throw new System.Exception ("property " + ListOfExpressions.Item(x).PropertyName + " does not exist!");
				}
			}

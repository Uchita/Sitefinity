...
namespace SitefinityWebApp.Mvc.Helpers
{
    ...
    #region
    /// <summary>
    /// This is custom helper class
    /// </summary>
    public class AKHelper
    {
        /// <summary>
        /// Get taxonmoy and sub taxonomy
        /// </summary>
        public static List<HierarchicalTaxon> GetCategorieTree(string categoryName = "")
        {
            var manager = TaxonomyManager.GetManager();
            var categoriesTaxonomy = manager.GetSiteTaxonomy<HierarchicalTaxonomy>(TaxonomyManager.CategoriesTaxonomyId, SystemManager.CurrentContext.CurrentSite.Id);
            HierarchicalTaxon hierarchicalTaxon = null;

            if (!string.IsNullOrEmpty(categoryName))
                hierarchicalTaxon = categoriesTaxonomy.Taxa.Where(t => t.Name == categoryName).FirstOrDefault() as HierarchicalTaxon;

            List<HierarchicalTaxon> listTaxon = new List<HierarchicalTaxon>();

            if (hierarchicalTaxon != null)
            {
                foreach (var taxon in hierarchicalTaxon.Subtaxa)
                {
                    listTaxon.Add(taxon);
                }
            }

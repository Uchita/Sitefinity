...
namespace SitefinityWebApp.Mvc.Models.CustomDynamicContent
{
    public class CustomDynamicContentModel : DynamicContentModel
    {
        ...
        protected override IEnumerable<ItemViewModel> ApplyListSettings(int page, IQueryable<IDataItem> query, out int? totalPages)
        {
            ...
            else if (this.ContentType.FullName == DynamicTypeConstants.Blog.FullTypeName)
            {
                if (HttpContext.Current.Items.Contains(BlogsTotalCountFlag))
                {
                    HttpContext.Current.Items[BlogsTotalCountFlag] = totalCount;
                }
                else
                {
                    HttpContext.Current.Items.Add(BlogsTotalCountFlag, totalCount);
                }


                if (HttpContext.Current.Items.Contains(BlogsItemsPerPageFlag))
                {
                    HttpContext.Current.Items[BlogsItemsPerPageFlag] = this.ItemsPerPage;
                }
                else
                {
                    HttpContext.Current.Items.Add(BlogsItemsPerPageFlag, this.ItemsPerPage);
                }
            }

            else if (this.ContentType.FullName == DynamicTypeConstants.Post.FullTypeName)
            {
                if (HttpContext.Current.Items.Contains(PostsTotalCountFlag))
                {
                    HttpContext.Current.Items[PostsTotalCountFlag] = totalCount;
                }
                else
                {
                    HttpContext.Current.Items.Add(PostsTotalCountFlag, totalCount);
                }


                if (HttpContext.Current.Items.Contains(PostsItemsPerPageFlag))
                {
                    HttpContext.Current.Items[PostsItemsPerPageFlag] = this.ItemsPerPage;
                }
                else
                {
                    HttpContext.Current.Items.Add(PostsItemsPerPageFlag, this.ItemsPerPage);
                }
            }
        ...
        {
            if (this.ContentType.FullName == DynamicTypeConstants.Article.FullTypeName || this.ContentType.FullName == DynamicTypeConstants.Blog.FullTypeName ||
                this.ContentType.FullName == DynamicTypeConstants.Post.FullTypeName)
            {
        ...
        public const string BlogsTotalCountFlag = "blogs-total-count";
        public const string BlogsItemsPerPageFlag = "blogs-items-per-page";

        public const string PostsTotalCountFlag = "posts-total-count";
        public const string PostsItemsPerPageFlag = "posts-items-per-page";

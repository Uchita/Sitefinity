...
;(function($) {
  ...
  var SlimTable = {
	...
	returnRowType: function(data) {
		var patt_01 = /[^0-9]/g,
			patt_02 = /^[0-9]+([\.,][0-9]+)?$/,
			patt_03 = /^([0-9]+([\.,][0-9]+)?)\s*[%$ۣe]?$/,
			patt_04 = /^[0-9]{1,2}[ \.\-\/]([0-9]{1,2}|[a-zA-Z]{3,9})[ \.\-\/][0-9]{4}$/;
		...
		for(l1=0; l1<this.state.tableHeads.length; l1++)
		{
			...
			for(l2=0; l2<this.state.tblData.length; l2++)
			{
				...
				if ( tAttr == 4 )
				{
					...
					var pattMonthName = /[a-zA-Z]{3,9}/;

					tObj = tObj.split(/[ .\/-]/);
					if( pattMonthName.test(tObj[1]) ){
						if( tObj[1].length == 3 ){
							tObj[1] = monthAbvrNames.indexOf(tObj[1].toLowerCase()) + 1;
						}else{
							tObj[1] = monthfullNames.indexOf(tObj[1].toLowerCase()) + 1;
						}
					}
   ...
   * ******************************************************************* */
$(function () {
    jQuery('img.icon-img').each(function () {
        var $img = jQuery(this);
